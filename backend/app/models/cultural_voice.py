"""
Cultural Voice Index (CVI) models for Nigerian linguistic patterns
"""
...
class CVIAnchor(BaseModel):
    """Individual Cultural Voice Index anchor"""
    phrase: str
...
class CulturalVoiceIndex(BaseModel):
    """Complete Cultural Voice Index entry"""
    id: str
...
class CVIGeneration(BaseModel):
    """CVI-enhanced generation parameters"""
    selected_anchors: List[CVIAnchor]
    voice_instructions: str
    cultural_constraints: List[str] = Field(default_factory=list)
    pidgin_level_adjustment: float = Field(ge=-1.0, le=1.0)
    formality_adjustment: float = Field(ge=-1.0, le=1.0)
    sentiment_bias: Optional[SentimentCategory] = None

class CVIEvaluation(BaseModel):
    """CVI evaluation metrics"""
