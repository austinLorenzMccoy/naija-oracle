...
- **Render free-tier memory ceiling.** The production backend runs on Render's 512 MB free tier. Initial deployments included `torch`, `sentence-transformers`, `transformers`, and `bert-score` in `requirements.txt`; importing these at startup alone consumed 300–400 MB, causing OOM restarts, 504 Gateway Timeouts, and truncated JSON responses. We resolved this by removing all heavy ML dependencies from the backend — the live API paths (review simulation and recommendations) rely entirely on the Groq API and never invoke local model inference, so the removal had no functional impact. Persona embeddings and NDCG ranking remain available locally via the standalone `task_b/` service. The tradeoff is that the Render-hosted backend cannot fall back to local embeddings if the Groq API is unreachable.  
