...
class RecommendationEngine:
    ...
    async def _map_to_persona_cluster(self, user_answers: Dict[str, Any]) -> Dict[str, Any]:
        """Map cold-start onboarding answers to a persona cluster"""
        food = str(user_answers.get("food_preference", "")).lower()
        entertainment = str(user_answers.get("entertainment_type", "")).lower()
        fintech = str(user_answers.get("fintech_app", "")).lower()

        # Simple heuristic: suya/street food → Lagos casual; afrobeats/shrine → expressive; kuda/tech → digital
        if "suya" in food or "street" in food:
            return {"id": "3", "name": "Tunde B.", "cluster": "Lagos casual"}
        elif "afrobeats" in entertainment or "shrine" in entertainment:
            return {"id": "1", "name": "Emeka O.", "cluster": "Lagos expressive"}
        elif "kuda" in fintech or "tech" in fintech:
            return {"id": "11", "name": "Zainab M.", "cluster": "Abuja digital"}
        elif "jollof" in food or "restaurant" in food:
            return {"id": "5", "name": "Biodun F.", "cluster": "Ibadan value"}
        else:
            return {"id": "1", "name": "Emeka O.", "cluster": "Lagos default"}
